...
export default function InputBarangModal({
	onCancel,
	onConfirm,
	data,
	table,
	...
	const [payload, setPayload] = useState(
		data
			? {
					...
					magnitudes: magnitudes.find(
						(m) => m.id === data.magnitudes_id
					).name_magnitudes,
					categories: categories.find(
						(c) => c.id === data.categories_id
					).name_categories,
					image: null,
					_method: "PUT",
			  }
	...
	const handleSubmit = (event) => {
		...
		if (validateInput()) {
			if (isAddNewMagnitude) {
				postMagnitudes(magnitudePayload)
					.then((res) => {
						setMagnitudes([...magnitudes, res.data]);
						setPayload({
							...payload,
							magnitudes: res.data.name_magnitudes,
						});
			...
			<Box
				...
				<div className="container">
					...
					<form onSubmit={handleSubmit}>
								...
								{!editQtyStatus.plus ? (
									<div
										onClick={() => {
											...
											payload.eligible_items > 0 && setPayload({
												...payload,
												eligible_items:
													payload.eligible_items - 1,
											});
								...
								{!editQtyStatus.plus ? (
									<div
										onClick={() => {
											...
											payload.defective_items > 0 && setPayload({
												...payload,
												defective_items:
													payload.defective_items - 1,
											});
						...
						{isAddNewMagnitude && (
							<div className="form-input">
								<label htmlFor="add_name_magnitudes">
									Magnitude Name
								</label>
								<input
